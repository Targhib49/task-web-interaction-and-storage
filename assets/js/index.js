...
/*          => Simpan update array "list" ke storage => Perlihatkan isi cart                        */
/*      5 - Masuk filterCart => Apakah user ingin memfilter cart?                                   */
/*          => (Ya: Masuk ke sistem filter data) | (Tidak: Menampilkan windows terima kasih)        */
/*          => Ya: masuk ke sistem filter data => menerima input kuantitas barang yang dicari       */
/*          => Memeriksa satu per satu index "list" kuantitas yang sama sesuai input                */
/*          => Menyimpan barang yang sesuai di array "temporary" => Menampilkan array "Temporary"   */
/*      6 - Masuk windows terima kasih => END                                                       */
/****************************************************************************************************/
...
let showCart = list => {
    ...
    if (list.length != 0) {
        ...
        for (let index = 0; index < list.length; index++) {
            cartResult += `${index + 1}. Item : ${list[index].item}, Quantity : ${list[index].quantity} \n`;
        }
        alert(cartResult);
...
let filterCart = list => {
    ...
    let isFilterCart = confirm(`Do you want to filter cart?`);
    if (isFilterCart === true){
        let qtyNumber = prompt(`Write the quantity you want to filter`);
        for (let i=0; i<list.length; i++) {
            if (list[i].quantity == qtyNumber){
                let filterProd = new Object();
                filterProd.item = list[i].item;
                filterProd.quantity = qtyNumber;
                newArr.push(filterProd);
            }
        }
        showCart(newArr);
...
//9 - Terima kasih
let thankYou = () => {
    return alert("Thank you for visiting us")
}
